...
html, body, .stApp, [data-testid="stAppViewContainer"], [data-testid="stHeader"] {
    ...
    direction: rtl; text-align: right; background-color: #F8FAFC !important; color: #1E293B !important;
}
[data-testid="stSidebar"] { display: none; }
#MainMenu, footer, header { visibility: hidden; }

.section-title {
    font-size: 20px; font-weight: 700; color: #10B981 !important;
    margin-bottom: 20px; padding-bottom: 12px;
    border-bottom: 2px solid rgba(16,185,129,0.35); display: inline-block;
}

.kpi-card {
    background: #FFFFFF; border-radius: 20px; padding: 26px 18px;
...
.stButton > button {
    ...
    background: #10B981 !important; color: #FFFFFF !important; border: none !important;
}
</style>
...
# (שאר הפונקציות נשארות כפי שהיו: get_discount, calculate_costs, optimize_split_purchase)
...
def calculate_costs(cart, prices, chains, promotions):
    ...
    for _, chain in chains.iterrows():
        ...
        for product_id, quantity in cart.items():
            ...
            covered += 1
            total += max((float(price_row.iloc[0]["price"]) * quantity) - get_discount(product_id, chain_id, float(price_row.iloc[0]["price"]) * quantity, promotions), 0)
...
with tab3:
    if st.button("הרץ אופטימיזציה", type="primary"):
        opt_total, opt_ok, assignment = optimize_split_purchase(st.session_state.cart, prices, chains, promotions)
        costs_df = calculate_costs(st.session_state.cart, prices, chains, promotions)
        single_cost = costs_df["עלות סל"].min()
        saving = round(single_cost - opt_total, 2)
        saving_pct = round(saving / single_cost * 100, 1)

        # פריסה של 3 כרטיסים על כל הרוחב
        st.markdown(f"""
