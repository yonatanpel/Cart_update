...
def determine_category(product_name):
    ...
    # 2. מוצרי ניקוי (העברנו לפה, כדי ש"פיירי" יזוהה לפני הכל)
    if any(k in name for k in ["אקונומיקה", "פיירי", "סבון כלים", "ניקוי", "שקיות אשפה"]): 
        return "מוצרי ניקוי"

    # 3. משקאות ואלכוהול (הוספנו את ג'ק דניאלס)
    ...
    # 4. מתוקים וחטיפים
    sweets_brands = ["עלית", "קינדר", "אוסם", "פרינגלס", "שטראוס", "נסטלה","פיטנס", "כרמית", "יוניליוור", "קורני", "קורנפלקס"]
    if any(k in name for k in sweets_brands + ["שוקולד", "שלגון", "במבה","חמצוצים", "ביסלי","דגני בוקר", "דגני","חטיפי", "עוגיות", "וופל", "חטיף", "גלידה"]):
        return "מתוקים וחטיפים"
        
    # 5. מזווה
    pantry_brands = ["סוגת", "יכין", "קנור", "היינץ", "אוסם", "תלמה", "וילי פוד", "שופרסל", "טעמן"] 
    pantry_keywords = ["בטעם עוף", "אורז", "מרק עוף", "בשמן", "גלטין", "פסטה", "קוסקוס", "טונה", "שימורים", "רסק", "רוטב", "סוכר", "חתיכות", "הרינג", "קמח", "מחית", "מצות", "שמן", "תבלין", "מיץ לימון", "סודה לשתייה", "חלב קוקוס", "לימונית", "מיץ 100%", "פלפל שחור", "מלח", "קטניות", "עדשים", "שעועית", "פירורי לחם", "טחינה", "קפה", "תה"]
