...
def determine_category(product_name):
    ...
    # 2. מוצרי ניקוי (פיירי/סבון כלים מוגדרים במדויק)
    if any(k in name for k in ["אקונומיקה", "פיירי", "סבון כלים", "ניקוי", "שקיות אשפה"]): 
        return "מוצרי ניקוי"

    # 3. משקאות ואלכוהול (כולל המותגים הספציפיים שלך)
    # שמתי את 'ג'ק דניאלס' בתור מותג, ואת 'תפוח' מחקתי מהפירות כדי שלא יתבלבל
    alcohol_brands = ["גולדסטאר", "היינקן", "קרלסברג", "טובורג", "אבסולוט", "פינלנדיה", "ג'וני ווקר", "ג'יימסון", "ג'ק דניאלס", "ערק", "כרמל", "ברקן", "רמת הגולן"]
    ...
    # 4. מזווה (הוספתי כאן את הלימונית, מיץ לימון ושיבולת שועל)
    pantry_brands = ["סוגת", "יכין", "קנור", "היינץ", "אוסם", "תלמה", "וילי פוד", "שופרסל", "טעמן"] 
    pantry_keywords = ["שיבולת שועל", "לימונית", "מיץ לימון", "אורז", "פסטה", "קוסקוס", "טונה", "שימורים", "רסק", "סוכר", "קמח", "שמן", "תבלין", "מלח", "קטניות", "טחינה", "קפה", "תה"]
    if any(k in name for k in pantry_brands + pantry_keywords):
        return "מזווה"

    # 5. ירקות (תפוח אדמה בלבד!)
    if any(k in name for k in ["תפוח אדמה", "עגבני", "מלפפון", "גזר", "פלפל"]):
        return "ירקות"

    # 6. פירות (בלי תפוח אדמה ובלי תפוח עץ אם הוא לא פרי)
    if any(k in name for k in ["תפוח עץ", "בננה", "תפוז", "אגס"]):
        return "פירות"

    # 7. מתוקים וחטיפים (סירופ עבר לכללי כמו שביקשת)
    sweets_brands = ["עלית", "קינדר", "אוסם", "פרינגלס", "שטראוס", "נסטלה", "כרמית"]
    if any(k in name for k in sweets_brands + ["שוקולד", "במבה", "ביסלי", "עוגיות", "וופל", "חטיף", "גלידה"]):
